...
def cnt_in_sentfile(sentences):
    txt_data =[]
    i =0
    for sent in sentences:
        i = i+1
        cnt= cnt_wordsfile(sent)
...
def calc_TFfile(text_data, freq_list):
    tf_score=[]
...
def calc_IDFfile(text_data, freq_list):
    idf_scores=[]
    cnt=0
    for item in freq_list:
        cnt= cnt +1
...
def calc_TFIDFfile(tf_scores, idf_scores):
    tfidf_scores=[]
    for j in idf_scores:
        for i in tf_scores:
...
def sent_scoresfile(tfidf_scores, sentences, text_data):
    sent_data=[]
